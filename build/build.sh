...
# Canonical (single source of truth): SKILL.md, SESSION-PREAMBLE.md,
# PROMPT-GATE.md, references/.
...
PROMPT_GATE="${ROOT}/PROMPT-GATE.md"
...
for agent in codex hermes; do
  ...
  cp "${PROMPT_GATE}" "${DIST}/${agent}/PROMPT-GATE.md"
done
...
cp "${ROOT}/hooks/hooks.json" "${ROOT}/hooks/session-start" "${ROOT}/hooks/user-prompt-submit" "${ROOT}/hooks/run-hook.cmd" "${DIST}/codex/hooks/"
...
cp "${PROMPT_GATE}" "${DIST}/hermes/agent-hooks/PROMPT-GATE.md"
...
sources at the repo root (`SKILL.md`, `SESSION-PREAMBLE.md`, `PROMPT-GATE.md`,
`references/`).
